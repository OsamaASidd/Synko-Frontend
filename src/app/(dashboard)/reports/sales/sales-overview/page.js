...
export default function Reports() {
  ...
  const [dateFrom, setDateFrom] = useState(result?.startDate);
  const [dateTo, setDateTo] = useState(result?.endDate);
  ...
  const get_SaleByMerchantData = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/sale_by_merchant?startDate=${dateFrom}&endDate=${dateTo}&search=${searchMerchantValue}&query=${query}`
    )
      .then((res) => {
        let merchatAry = [];
  ...
  const get_RushHourCategoryBar = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/top/category/peekhours/bar?startDate=${dateFrom}&endDate=${dateTo}&query=${query}`
    )
      .then((res) => {
        if (res?.data?.result.length != 0) {
  ...
  const get_RushHourCategoryGage = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/top/category/peekhours/gage?startDate=${dateFrom}&endDate=${dateTo}&categorySearch=${searchTrendingCategories}&query=${query}`
    )
      .then((res) => {
        if (res?.data?.result.length != 0) {
  ...
  const get_RushHour = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/top/category/peekhours?startDate=${dateFrom}&endDate=${dateTo}&search=${searchRushHour}&query=${query}`
    )
      .then((res) => {
        let merchatAry = [];
  ...
  const getTopItems = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/top/item/sales?startDate=${dateFrom}&endDate=${dateTo}&searchTopItems=${searchTopItems}&query=${query}`
    )
      .then((res) => {
        let itemsAry = [];
  ...
  const topCategories = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/top/category/sales?startDate=${dateFrom}&endDate=${dateTo}&searchTopCategory=${searchTopCategory}&query=${query}`
    )
      .then((res) => {
        let catAry = [];
  ...
  const getSaleByCategoryData = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/sale_by_category?startDate=${dateFrom}&endDate=${dateTo}&search=${searchByCategoryValue}&query=${query}`
    )
      .then((res) => {
        let catAry = [];
  ...
  const dispatchTypeSell = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/dispatch_type?startDate=${dateFrom}&endDate=${dateTo}&query=${query}`
    )
      .then((res) => {
        let dispatchs = [];
  ...
  const getSalesOverView = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/sales_overview?startDate=${dateFrom}&endDate=${dateTo}&query=${query}`
    )
      .then((res) => {
  ...
  const getSalesItemSummaryOverviw = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/sales_item_summary?startDate=${dateFrom}&endDate=${dateTo}&query=${query}`
    )
      .then((res) => {
  ...
  const saleByEmployeeData = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/sale_employee?startDate=${dateFrom}&endDate=${dateTo}&search=${searchByEmployeeValue}&query=${query}`
    )
      .then((res) => {
  ...
  const SaleByPaymentMethods = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/sale_payment_method?startDate=${dateFrom}&endDate=${dateTo}&query=${query}`
    )
      .then((res) => {
  ...
  const getSalesItemSummary = () => {
    setLoading(true);
    getRequest(
      `/merchant/${merchant?.id}/sales_by_item?startDate=${dateFrom}&endDate=${dateTo}&search=${searchSaleByItem}&query=${query}`
    )
      .then((res) => {
  ...
  }, [user, merchant, searchMerchantValue]);
  ...
  }, [user, merchant, searchSaleByItem]);
  ...
  }, [user, merchant, searchByCategoryValue]);
  ...
  }, [user, merchant, searchByEmployeeValue]);
  ...
  }, [user, merchant, searchTopCategory]);
  ...
  }, [user, merchant, searchTopItems]);

  useEffect(() => {
    if (user !== null && merchant !== null) {
      get_RushHour();
    }
  }, [user, merchant, searchRushHour]);

  useEffect(() => {
    if (user !== null && merchant !== null) {
      get_RushHourCategoryGage();
    }
  }, [user, merchant, searchTrendingCategories]);
  ...
  const handleChaneDateRange = async (value) => {
    let { startDate, endDate } = await getDateRange(value);
      ...
      <div className=" w-[100%] xl:pl-0 xl:w-[80%] px-5 xl:px-0 xl:pr-5 py-6  text-black bg-[#171821]">
        <div className=" px-4 md:px-10 rounded-lg py-12 bg-gray-50 w-[100%] h-[calc(100vh-48px)] overflow-y-auto">
          ...
          <div className="flex flex-col w-[100%]">
            <div className="w-[100%] flex flex-col lg:flex-row justify-between  gap-2 lg:items-center my-[10px] ">
              <div className=" ">
                <p className="text-[18px] md:text-[24px]  font-[500] ">
                  Overview
                </p>
              </div>
              ...
              <div className="px-4">
                <select
                  id="sort-by"
                  name="sort_sale_payment_by"
                  onChange={(e) => {
                    handleChaneDateRange(e.target.value);
                  }}
              ...
              <div className="flex flex-col lg:flex-row gap-4  lg:items-center">
                <div className="flex space-x-4 justify-between lg:justify-normal items-center ">
            ...
            {dateFrom && dateTo ? (
              <>
                {/* tabs */}
                <div className="flex flex-wrap border-b border-gray-400 mt-5">
                              ...
                              return (
                                ...
                                >
                                  <td className="px-7 py-2 lg:py-2 whitespace-nowrap">
                                    <Link
                                      href={`/reports/sales/sales-overview/product/${item?.name.replace(
                                        / /g,
                                        "-"
                    ...
                    <div className="text-white text-center p-4 bg-gradient-to-r from-[#7DE143] to-[#055938] rounded-10 flex justify-between mt-5">
                      <div className="flex ml-auto">
                        <input
                          className="border p-2 rounded-[10px] bg-[#F8F8F8] outline-none text-black"
                          placeholder="Search..."
                          value={searchTrendingCategories}
                          onChange={(e) => {
                            setSearchTrendingCategories(e.target.value);
                          }}
                        ></input>
