...
export default function TodayAttendance({
  ...
  const getData = () => {
    setLoading(true);
    let url = `/merchant/${merchant?.id}/get-attendance/employee?dateFrom=${dateRange[0]}&dateTo=${dateRange[1]}`;
      ...
      .then((res) => {
        ...
        if (res?.data && res?.data?.length > 0) {
          const formattedData = res?.data?.map((row) => ({
            ...
            "Check In DateTime": row.checkin_time
              ? moment.utc(row.checkin_time).format("LT, MMM DD, YYYY")
              : "",
            "Check Out DateTime": row.checkout_time
              ? moment.utc(row.checkout_time).format("LT, MMM DD, YYYY")
              : "",
            "Total Working Time": formatTime(row.total_minutes),
            "Total Payable Time": formatTime(
              calculatePayableTime(row.total_minutes)
            ),
            "Total Break Time": formatTime(
              row.break_minutes
              // calculateBreakTime(row.total_minutes)
            ),
          }));
  ...
  const TotalWorkingTimeCell = ({ rowData, dataKey, ...props }) => {
    const totalMinutes = rowData[dataKey];
    let adjustedMinutes = totalMinutes;
  ...
  const TotalBreakTimeCell = ({ rowData, dataKey, ...props }) => {
    ...
    return <Cell {...props}>{formatTime(breakMinutes)}</Cell>;
  };
            ...
            {!data ? (
              ...
              <>
                ...
                <Table autoHeight data={data} onRowClick={(rowData) => {}}>
                  ...
                  <Column width={200}>
                    <HeaderCell>Check In DateTime</HeaderCell>
                    <CheckTime dataKey="checkin_time" />
                  </Column>

                  <Column width={200}>
                    <HeaderCell>Check Out DateTime</HeaderCell>
                    <CheckTime dataKey="checkout_time" />
                  </Column>

                  <Column width={150}>
                    <HeaderCell>Total Working Time</HeaderCell>
                    <TotalWorkingTimeCell dataKey="total_minutes" />
                  </Column>

                  <Column width={150}>
                    <HeaderCell>Total Payable Time</HeaderCell>
                    <TotalPayableTimeCell dataKey="adjusted_minutes" />
                  </Column>

                  <Column width={150}>
                    <HeaderCell>Total Break Time</HeaderCell>
                    <TotalBreakTimeCell dataKey="break_minutes" />
                  </Column>
